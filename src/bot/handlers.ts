...
import { isAuthorized, sendToTopic, notifyUnauthorized } from "./helpers.js";
...
export function registerHandlers(bot: Telegraf, queue: PrQueue, awsBrowser?: AWSBrowser): void {
  ...
  bot.on(message("text"), async (ctx) => {
    ...
    if ((isApprove || isReject) && isExactCommand) {
      if (!isAuthorized(ctx.from?.username)) {
        await notifyUnauthorized(
          bot.telegram, ctx.from?.id, ctx.from?.username, ctx.chat.id,
    ...
    for (const prInfo of added) {
      ...
      analyzeAndRequestApproval(bot, awsBrowser, ctx.chat.id, prInfo, queue).catch((e: unknown) => {
